            ...
            steps {
                sh """
                    ssh -o StrictHostKeyChecking=no ubuntu@10.0.2.242 "
                        ...
                        docker run -d --name task-nodejs -e DB_HOST=${RDS_HOST} -e DB_USER=${RDS_USER} -e DB_PASSWORD=${RDS_PASSWORD} -e DB_NAME=${DB_NAME} -p 3001:3001 ${ECR_REGISTRY}/task-manager-nodejs:latest
                        
                        docker run -d --name task-fastapi -e DB_HOST=${RDS_HOST} -e DB_USER=${RDS_USER} -e DB_PASSWORD=${RDS_PASSWORD} -e DB_NAME=${DB_NAME} -p 8000:8000 ${ECR_REGISTRY}/task-manager-fastapi:latest
                        
                        docker run -d --name task-springboot -e SPRING_DATASOURCE_URL=\"jdbc:mysql://${RDS_HOST}:3306/${DB_NAME}?useSSL=false\" -e SPRING_DATASOURCE_USERNAME=${RDS_USER} -e SPRING_DATASOURCE_PASSWORD=${RDS_PASSWORD} -p 8080:8080 ${ECR_REGISTRY}/task-manager-springboot:latest
                        
                        docker run -d --name task-dotnet -e ASPNETCORE_URLS=\"http://+:5000\" -e ConnectionStrings__DefaultConnection=\"Server=${RDS_HOST};Database=${DB_NAME};User=${RDS_USER};Password=${RDS_PASSWORD};\" -p 5000:5000 ${ECR_REGISTRY}/task-manager-dotnet:latest
